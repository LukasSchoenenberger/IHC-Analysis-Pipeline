...
import matplotlib
matplotlib.use('Agg')  # Use non-interactive backend (safe for subprocess / no-display environments)
...
def select_input_files():
    ...
    Uses zenity if available, falls back to PyQt5.
    """
    ...
    if has_zenity:
        cmd = ['zenity', '--file-selection', '--multiple',
               ...
               f'--filename={Path.cwd()}/_',
               '--file-filter=NIfTI files | *.nii *.nii.gz *.gz',
               '--file-filter=All files | *']
        try:
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode == 0 and result.stdout.strip():
                paths = result.stdout.strip().split('|')
                print(f"File dialog closed. Selected {len(paths)} files.")
                return paths
        except Exception:
            pass
        return []
    ...
    paths, _ = QFileDialog.getOpenFileNames(
        None, "Select Segmentation Files to Split",
        str(Path.cwd()),
        "NIfTI files (*.nii *.nii.gz *.gz);;All files (*)",
...
def process_component(component, current_label, target_size=500):
    ...
    n_seeds = max(1, n_pixels // target_size)
...
def process_selected_files(file_paths, target_size=500):
    """Process the selected segmentation files"""
    if not file_paths:
        print("No files selected.")
        return False

    print(f"Using patch size: {target_size} pixels")

    # Always save to Split_Annotation_Masks in current working directory
    output_folder = Path.cwd() / "Split_Annotation_Masks"
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            seg_labels = np.unique(slice_data[slice_data > 0])
            result = np.zeros_like(data)
            current_label = 1

            print(f"Found {len(seg_labels)} segmentation label(s): {seg_labels.astype(int).tolist()}")

            for seg_label in seg_labels:
                label_mask = slice_data == seg_label
                labeled_components, n_components = ndimage.label(label_mask)
                print(f"  Label {int(seg_label)}: {n_components} connected component(s)")

                for comp_idx in range(1, n_components + 1):
                    print(f"  Processing label {int(seg_label)} component {comp_idx}/{n_components}")
                    ...
                    steps_output_path = steps_folder / f"{file_path.stem}_label{int(seg_label)}_component{comp_idx}_steps.png"
                    visualize_steps(
                        component,
                        potential_seeds,
                        seed_points,
                        initial_regions,
                        final_regions,
                        f"Processing Steps for Label {int(seg_label)} Component {comp_idx}",
                        steps_output_path
                    )
                    ...
                    current_label = int(result.max()) + 1

            # Save final result
            new_img = nib.Nifti1Image(result, img.affine)
            output_path = output_folder / f"{file_path.stem}_kmeans_split_patchsize_{target_size}.nii.gz"
...
def process_segmentations(folder_path, output_folder=None, file_suffix="_bin.nii.gz", target_size=500):
    ...
    if output_folder is None:
        ...
        output_folder = folder_path.parent / "Split_Annotation_Masks"
    else:
        ...
        try:
            ...
            for seg_label in seg_labels:
                ...
                for comp_idx in range(1, n_components + 1):
                    ...
                    # Create visualization for every component
                    steps_output_path = steps_folder / f"{nifti_file.stem}_label{int(seg_label)}_component{comp_idx}_steps.png"
                    visualize_steps(
                        component,
                        potential_seeds,
                        seed_points,
            ...
            # Save final result
            new_img = nib.Nifti1Image(result, img.affine)
            output_path = output_folder / f"{nifti_file.stem}_kmeans_split_patchsize_{target_size}.nii.gz"
            nib.save(new_img, str(output_path))
            print(f"Saved split segmentation to {output_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--patchsize", type=int, default=500,
                        help="Target patch size in pixels (default: 500)")
    parser.add_argument("--files", type=str, default=None,
                        help="Comma-separated list of specific file paths to process (batch mode)")

    args = parser.parse_args()

    if args.files:
        # Batch mode: process specific files directly, no dialog
        selected_files = [f.strip() for f in args.files.split(",") if f.strip()]
        if selected_files:
            print(f"Batch mode: processing {len(selected_files)} file(s):")
            for file_path in selected_files:
                print(f"  - {Path(file_path).name}")
            success = process_selected_files(selected_files, target_size=args.patchsize)
            if success:
                print("\nProcessing completed successfully!")
                print(f"Results saved to: {Path.cwd() / 'Split_Annotation_Masks'}")
        else:
            print("No files specified. Exiting.")

    elif args.gui or (args.input_folder is None):
        ...
        if args.output_folder is None:
            # Use script's directory as base directory
            base_dir = Path(__file__).parent
            output_folder = base_dir / "Split_Annotation_Masks"
        else:
        ...
        print(f"Patch size: {args.patchsize}")

        process_segmentations(input_folder, output_folder, args.suffix, target_size=args.patchsize)

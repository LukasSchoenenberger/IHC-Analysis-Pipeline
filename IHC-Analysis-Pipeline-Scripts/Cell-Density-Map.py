...
# -*- coding: utf-8 -*-
"""
Cell Density Map Script
-----------------------
This script generates density maps from cell detection results.
It reads cell count data from CSV and creates two TIFF density maps:
1. Raw cell counts per tile
2. Normalized cell counts (0-255 range)
...
# Configure logging
logger = logging.getLogger("Cell-Density-Map")

class CellDensityMapGenerator:
    """
    Class for generating density maps from cell detection results
    """
    
    def __init__(self, data_dir, output_dir, parameters_dir):
        """
        Initialize the CellDensityMapGenerator
        ...
        # Set input CSV path
        self.csv_path = self.output_dir / "Cell-Detection" / "cell_count.csv"
        ...
        logger.info(f"Initialized Cell Density Map Generator")
    ...
    def find_count_column(self, df):
        """
        Find the correct cell count column name in the DataFrame
        
        Args:
            df (pandas.DataFrame): DataFrame containing tile data
            
        Returns:
            str: Column name for cell counts or None if not found
        """
        # Check for different case variants of cell count column
        potential_columns = [
            'cell_count',      
            'Cell_Count',
            'count',
        ...
        for col in potential_columns:
            if col in df.columns:
                print(f"Using cell count column: {col}")
                sys.stdout.flush()
                logger.info(f"Using count column: {col}")
                return col
        
        logger.error(f"Could not find cell count column in DataFrame")
        logger.error(f"Available columns: {df.columns.tolist()}")
        return None
    
    def generate_density_maps(self):
        """
        Generate both raw and normalized density maps for cell counts
        
        Returns:
            bool: True if successful, False otherwise
        """
        try:
            print("Starting cell density map generation...")
            sys.stdout.flush()
            logger.info(f"Generating cell density maps from {self.csv_path}")
            
            # Check if the CSV file exists
            if not self.csv_path.exists():
                print(f"ERROR: Cell detection results CSV file not found at {self.csv_path}")
                sys.stdout.flush()
                logger.error(f"CSV file not found at {self.csv_path}")
                return False
            
            print("Reading cell detection results...")
            sys.stdout.flush()
            
            # Read the CSV file
            df = pd.read_csv(self.csv_path)
            
            print(f"Loaded {len(df)} cell detection results")
            sys.stdout.flush()
            
            # Find the correct count column
            count_col = self.find_count_column(df)
            if not count_col:
                print("ERROR: Could not find cell count column in CSV")
                sys.stdout.flush()
                return False
            ...
            print(f"Cell count range: {min_count} to {max_count}")
            sys.stdout.flush()
            ...
            if max_count == min_count:
                print("WARNING: All cell counts are identical, creating uniform normalized density map")
                sys.stdout.flush()
                df['Count_Normalized'] = np.full(len(df), 128, dtype=np.uint8)  
            else:
            ...
            print(f"ERROR: Failed to generate cell density maps: {str(e)}")
            sys.stdout.flush()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate cell density maps from detection results')
    ...
    log_file = os.path.join(log_dir, "cell_density_map.log")
    ...
    logger.info("Starting Cell Density Map Generation")
    logger.info(f"Data directory: {args.data_dir}")
    logger.info(f"Output directory: {args.output_dir}")
    logger.info(f"Parameters directory: {args.parameters_dir}")
    
    # Create generator instance
    generator = CellDensityMapGenerator(
    ...
    if success:
        print("Cell density map generation completed successfully")
        sys.stdout.flush()
        logger.info("Cell density map generation completed successfully")
        return 0
    else:
        print("ERROR: Failed to generate cell density maps")
        sys.stdout.flush()
        logger.error("Failed to generate cell density maps")
        return 1
